...
Before you can push your changes to the MLReef server
...
MLReef supports RSA, DSA, ECDSA, and ED25519 keys. Their difference lies on
...
If for any reason your OS or the MLReef instance you interact with doesn't
support ED25519, you can fallback to RSA.

NOTE: **Note:**
Omnibus does not ship with OpenSSH, so it uses the version on your MLReef server. If using
...
servers that may have an old OpenSSH version. Use them if the MLReef server
...
If you already have an RSA SSH key pair to use with MLReef, consider upgrading it
...
Now, it's time to add the newly created public key to your MLReef account.

## Adding an SSH key to your MLReef account
...
1. Add your **public** SSH key to your MLReef account by:
...
your terminal (replacing `mlreef.com` with your MLReef's instance domain):

```bash
ssh -T git@mlreef.com
```

The first time you connect to MLReef via SSH, you will be asked to verify the
authenticity of the MLReef host you are connecting to.
For example, when connecting to MLReef.com, answer `yes` to add MLReef.com to
the list of trusted hosts:

**CHECK**!!!!
```
The authenticity of host 'mlreef.com (35.231.145.151)' can't be established.
...
authenticity of MLReef's host again. Run the above command once more, and
you should only receive a _Welcome to MLReef, `@username`!_ message.
...
If you used a non-default file path for your MLReef SSH key pair,
you must configure your SSH client to find your MLReef private SSH key
for connections to MLReef.
...
file you can set up configurations for multiple hosts, like MLReef.com, GitHub, Bitbucket, etc.
...
Host mlreef.com
  ...
  IdentityFile ~/.ssh/mlreef_com_rsa
```

Public SSH keys need to be unique to MLReef, as they will bind to your account.
...
## Multiple accounts on a single MLReef instance

The [per-repository](#per-repository-ssh-keys) method also works for using
multiple accounts within a single MLReef instance.
...
Host <user_1.mlreef.com>
  Hostname mlreef.com
  PreferredAuthentications publickey
  IdentityFile ~/.ssh/<example_ssh_key1>

# User2 Account Identity
Host <user_2.mlreef.com>
  Hostname mlreef.com
  PreferredAuthentications publickey
...
NOTE: **Note:**
The example `Host` aliases are defined as `user_1.mlreef.com` and
`user_2.mlreef.com` for efficiency and transparency. Advanced configurations
...
Cloning the `mlreef` repository normally looks like this:

```sh
git clone git@mlreef.com:mlreef-org/mlreef.git
```

To clone it for `user_1`, replace `mlreef.com` with the SSH alias `user_1.mlreef.com`:

```sh
git clone git@<user_1.mlreef.com>:mlreef-org/mlreef.git
```
...
```sh
git remote set-url origin git@<user_1.mlreef.com>:mlreef-org/mlreef.git
```
...
## SSH on the MLReef server

**CHECK**!!!

MLReef integrates with the system-installed SSH daemon, designating a user
(typically named `git`) through which all access requests are handled. Users
connecting to the MLReef server over SSH are identified by their SSH key instead
of their username.

SSH *client* operations performed on the MLReef server wil be executed as this
...
The MLReef check process includes a check for this condition, and will direct you
...
If on Git clone you are prompted for a password like `git@mlreef.com's password:`
something is wrong with your SSH setup.

- Ensure that you generated your SSH key pair correctly and added the public SSH
  key to your MLReef profile
- Try manually registering your private SSH key using `ssh-agent` as documented
  earlier in this document
- Try to debug the connection by running `ssh -Tv git@example.com`
  (replacing `example.com` with your MLReef domain)
